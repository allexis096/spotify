...
final class AuthManager {
    ...
    struct Constants {
        ...
        static let redirectUri = "http://google.com"
    ...
    public var signInUrl: URL? {
        ...
        let string = "\(baseUrl)?response_type=code&client_id=\(Constants.clientID)&scope=\(Constants.scopes)&redirect_uri=\(Constants.redirectUri)&state=\(Utils.shared.randomString(length: 16))&show_dialog=true"
    ...
    public func refreshIfNeeded(completion: @escaping (Bool) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            ...
            do {
                let result = try JSONDecoder().decode(AuthResponse.self, from: data)
                print("resultRefreshToken: \(result)")
                self?.cacheToken(result: result)
                completion(true)
            } catch {
                print(error.localizedDescription)
